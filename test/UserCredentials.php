...
if( count($argv) > 1 )
{
	for( $x = 1; $x < count($argv); $x++ )
	{
		switch($argv[$x])
		{
			...
			case '--username': { //Sets the value for $username
				$username = trim($argv[$x+1]);
			}; break;
			case '--password': { //Sets the value for $password
...
if ( count($argv) == 10 ) //Checks if all the parameters are entered
{
	LocalEcho("Processing User Credentials: \n\n");
	...
	$result = $api_test->userCredentials($username, $password); //Returns a boolean for success or fail API call
